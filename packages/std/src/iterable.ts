 ...
 * @returns
 */
export const filterIterable = <T>(elements: Iterable<T>, predicate: IPredicate<T>): T[] => {
  const list: T[] = []
  let i = 0
  for (const value of elements) {
    if (predicate(value, i)) list.push(value)
    i += 1
  }
  return list
}
 ...
 * @returns
 */
export const mapIterable = <T, R>(elements: Iterable<T>, map: IMapper<T, R>): R[] => {
  const list: R[] = []
  let i = 0
  for (const value of elements) {
