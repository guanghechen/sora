...
**覆盖规则**：用户定义同名 `long` 时，内置选项定义与默认行为均被移除，用户需自行在 `apply` 或 `action` 中处理逻辑。
...
路由从左到右扫描 argv，遇到 `-` 或 `--` 开头的 token 即停止，后续全部视为当前命令的 options/args。这消除了 routing 与 option 定义之间的循环依赖。
