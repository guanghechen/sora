...
> {
  ...
  opts: TOpts                       // 解析后的选项（强类型推导）
  args: TArgs                       // 解析后的位置参数（强类型推导）
...
type ICommandAction<
  TOpts extends Record<string, unknown> = Record<string, unknown>,
  TArgs extends Record<string, unknown> = Record<string, unknown>,
> = (params: ICommandActionParams<TOpts, TArgs>) => void | Promise<void>
...
3. `action` 的 `opts/args` 目标语义是强类型推导（由 option/argument 定义得到）。
...
2. `.parse(params)` 返回 `Promise<ICommandParseResult<InferredOpts, InferredArgs>>`。
3. 两者在解析错误（格式错误、校验失败、unknown option 等）时都应抛错。
...
其中 `InferredOpts/InferredArgs` 表示由当前 command 本地声明的 options/arguments 推导得到的静态类型。

类型推导（II：仅 leaf 本地声明）示例：
...
const deploy = new Command({ desc: 'Deploy' })
  ...
  .action(({ opts, args }) => {
    opts.force      // boolean（本地声明）
    args.target     // string（本地声明）
    // opts.verbose // TS Error：祖先项不在 leaf 的对外 opts 类型中
  })
