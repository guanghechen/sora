...
// e.g., github.com/guanghechen/sora/tree/@guanghechen/std@2.0.0/...
const GITHUB_URL_PATTERN = new RegExp(
  String.raw`(github\.com/guanghechen/sora/tree/)(@[\w-]+/[\w-]+@)(${SEMVER})`,
  'g',
)
...
function updateContent(content, versionMap) {
  return content.replace(GITHUB_URL_PATTERN, (match, prefix, pkgWithAt) => {
    // pkgWithAt is like "@guanghechen/std@", extract package name
    const pkgName = pkgWithAt.slice(0, -1) // Remove trailing @
